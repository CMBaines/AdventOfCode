...
class Game:
    def __init__(self, gameStr):
        self.GameId = int(re.search("Game ([\d]*)", gameStr).group(1))
        self.MaxRed = max(map(int, re.findall("([\d]*) red", gameStr)))
        self.MaxGreen = max(map(int, re.findall("([\d]*) green", gameStr)))
        self.MaxBlue = max(map(int, re.findall("([\d]*) blue", gameStr)))
        self.Power = self.MaxRed * self.MaxGreen * self.MaxBlue

def part1FindPossibleGames(totalRed, totalGreen, totalBlue, games): 
    validGameIds = []
    for game in games:
        if (game.MaxRed <= totalRed and game.MaxGreen <= totalGreen and game.MaxBlue <= totalBlue):
...
def printSums():
    games = []
    for gameStr in input.splitlines():
        games.append(Game(gameStr))
    part1Ids = part1FindPossibleGames(12, 13, 14, games)
    part1Sum = sum(part1Ids)
    print(part1Sum)

    part2Sum = sum([game.Power for game in games])
    print(part2Sum)
