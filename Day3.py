...
class PartNum:
    def __init__(self, num, line, maxLine, startIdx, endIdx, maxIdx):
        self.PartNum = int(num)
        self.PossibleMatchLines = PartNum.findMatchLines(line, maxLine)
        self.PossibleMatchIndexes = PartNum.findMatchIndexes(startIdx, endIdx, maxIdx)

    @staticmethod
    def findMatchLines(line, maxLine):
        lines = []
    ...
    @staticmethod
    def findMatchIndexes(startIdx, endIdx, maxIdx):
        indexes = []

        if (startIdx != 0):
            indexes.append(startIdx - 1)

        indexes = indexes + list(range(startIdx, endIdx + 1))
...
class GearPart:
    def __init__(self, partNum, gear):
...
def printSums():
    ...
    for i, line in enumerate(split):
        partMatches = re.finditer("\d+", line)
        for partMatch in partMatches:
            parts.append(PartNum(partMatch.group(), i, maxLine, partMatch.start(), partMatch.end(), maxChar))
        charMatches = re.finditer("[^\d\.]", line)
        for charMatch in charMatches:
            chars.append(SpecialChar(i, charMatch.start()))
        gearMatches = re.finditer("\*", line)
        for gearMatch in gearMatches:
            gears.append(SpecialChar(i, gearMatch.start()))
    
    part1Sum = 0
    for char in chars:
        part1Sum = part1Sum + sum([part.PartNum for part in parts if char.Line in part.PossibleMatchLines and char.Index in part.PossibleMatchIndexes])
    ...
    gearRatios = []
    for gear in gears:
        possibleGearRatios[gear] = [GearPart(part.PartNum, gear) for part in parts if gear.Line in part.PossibleMatchLines and gear.Index in part.PossibleMatchIndexes]
        if (len(possibleGearRatios[gear]) == 2):
            gearRatios.append(math.prod([gearPart.PartNum for gearPart in possibleGearRatios[gear]]))
    
    part2Sum = sum(gearRatios)

    print(part2Sum)    

    return

input = """........936..672.........846.922........359...332......582..856........................579..93......674..740.....243.156....................
...........%.........4=...*...*........*.......*......#....................806..481.........................*.......*.........900......$564.
